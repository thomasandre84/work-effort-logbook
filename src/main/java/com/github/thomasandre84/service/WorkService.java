...
public class WorkService {
    ...
    @Transactional
    public WorkTimeDto createWorkTime(UpdateWorkTimeDto updateWorkTimeDto) {
        Work work = workRepository.findById(updateWorkTimeDto.workId());
        WorkTime workTime = new WorkTime();
        workTime.setWork(work);
        workTime.setStartTime(updateWorkTimeDto.startTime());
        if (updateWorkTimeDto.endTime() != null) {
    ...
    public Long getWorkTimeSummary(UUID id) {
        Work work = workRepository.findById(id);
        List<WorkTime> workTimes = workTimeRepository.findByWork(work);
        Optional<Long> totalMinutes = workTimes.stream()
                .map(WorkService::calcWorkingMinutes)
                .reduce(Long::sum);
        return totalMinutes.orElse(null);
    }
